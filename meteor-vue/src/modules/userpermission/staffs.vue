                                    ...
                                    <tr v-for="(row,index) in menus[0]">
                                        ...
                                        <td align="center">
                                            <button type="button" class="btn btn-sm btn-alt"
                                                    @click="modifyfun(row.id,row.name,row.group.id,row.sex,row.telno,row.comment)">
                                                修改
                                            </button>
                                            <button type="button" class="btn btn-sm btn-alt"
                                                    @click="delfun(row.id,index)">删除
                                            </button>
                                        </td>
                                ...
                                <div class="btn-group">
                                    ...
                                    <popdialog :show="show" @modifydata="modifydata">
                                        <form class="form-inline" role="form">
                                            <div class="form-group"><br><br><input style="display: none" id="mid">
                                                <lable class="label" style="text-align: right">用户组：</lable>
                                                <select class="form-control" style="width: 175px" id="groupid">
                                                    <option v-for="(row,index) in groups[0]" :value="row.id"
                                                            :selected="row.id==groupid ? 'selected':''">{{row.name}}
                                                    </option>
                                                </select><br><br>
                                                <lable class="label" style="text-align: right">用户名：</lable>
                                                <input class="form-control" id="username"><br><br>
                                                <lable class="label" style="text-align: right">性别：</lable>
                                                <input class="form-control" id="sex"><br><br>
                                                <lable class="label" style="text-align: right">电话：</lable>
                                                <input class="form-control" id="telno"><br><br>
                                                <lable class="label" style="text-align: right">备注：</lable>
                                                <input class="form-control" id="comment">
                                            </div>
                                        </form>
                                        <br><br>
                                        <button type="button" class="btn btn-sm btn-alt" @click="savefun()">保存</button>
                                        <button type="button" class="btn btn-sm btn-alt" @click="concelfun()">取消
                                        </button>
                                    </popdialog>
                                </div>
                            </div>
    ...
    export default {
        ...
        methods: {
            ...
            savefun: function () {
                ...
                if (mid != '') {
                    var url = config.baseurl + '/userpermission/staffmodify/' + mid;
                    var methods = 'PUT'
                }
                else {
                    var url = config.baseurl + '/userpermission/staffs/';
                    var methods = 'post'
                }
